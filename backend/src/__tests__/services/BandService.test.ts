...
describe('BandService', () => {
  ...
  describe('updateBand', () => {
    it('should update band with partial data', async () => {
      ...
      const result = await bandService.updateBand('band-123', updateData, owner);

      expect(result.description).toBe('Updated description');
      expect(result.genre).toBe('Alternative Rock');
      expect(mockDb.query).toHaveBeenCalledWith(
        expect.stringContaining('UPDATE bands'),
        ['Updated description', 'Alternative Rock', 'band-123', owner.id, false]
      );
    ...
    it('should throw error when no valid fields to update', async () => {
      await expect(bandService.updateBand('band-123', {}, owner)).rejects.toThrow(
    ...
    it('should ignore undefined values in update data', async () => {
      ...
      await bandService.updateBand('band-123', updateData, owner);
    ...
    it('binds admin actor so unclaimed bands can be updated', async () => {
      ...
      await bandService.updateBand('band-123', { name: 'Admin Updated' }, admin);

      const [query, params] = mockDb.query.mock.calls[0];
      expect(query).toContain('claimed_by_user_id = $3 OR $4::boolean');
      expect(params).toEqual(['Admin Updated', 'band-123', admin.id, true]);
    });
  });
  ...
  describe('deleteBand', () => {
    it('should soft delete band successfully', async () => {
      ...
      await bandService.deleteBand('band-123', owner);
      ...
      expect(deleteParams).toEqual(['band-123', owner.id, false]);
    });
    ...
    it('should deny pending verification claims on delete', async () => {
      ...
      await bandService.deleteBand('band-123', admin);
      ...
      expect(mockDb.query.mock.calls[0][1]).toEqual(['band-123', admin.id, true]);
    });
    ...
    it('should handle database errors during deletion', async () => {
      ...
      await expect(bandService.deleteBand('band-123', owner)).rejects.toThrow('Delete failed');
    });
  });
